#!/usr/bin/env python3
"""Generate Stackz's mod menu icon: a chest front, composed from the vanilla
chest entity texture.
...
OUT = os.path.join(HERE, "src/main/resources/assets/stackz/icon.png")
...
def build_icon():
    """A chest front, cut from the chest entity texture: lid face, base face
    and the latch straddling the seam. The mod is about how much fits in a
    container, so a container is the icon."""
    chest = vanilla("entity/chest/normal.png")
    canvas = blank(16, 16)
    blit(canvas, chest, 1, 1, sx=14, sy=14, w=14, h=5)     # lid front face
    blit(canvas, chest, 1, 6, sx=14, sy=33, w=14, h=10)    # base front face
    blit(canvas, chest, 7, 4, sx=1, sy=1, w=2, h=4)        # latch
